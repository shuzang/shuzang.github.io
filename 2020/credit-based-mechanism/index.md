...
设备 $i$ 信誉值由两部分组成，合法行为的正面影响和恶意行为的负面影响，如下公式，其中，$Cr_i^P$ 为正面部分，$Cr_i^N$ 为负面部分，$\lambda_1$ 和 $\lambda_2$ 分别是它们的权重。
$$
...
$$
Cr_i^P = min({Cr_i^P}\text{max},  \sum_{k=k_1}^{l_i-1} \omega_k)
$$
其中，${Cr_i^P}_{max}$ 是 $Cr_i^P$ 的上界，防止奖励无限制积累，$l_i$ 为设备 $i$ 的合法行为总数，$k_1$ 是上一次做出惩罚时计算的最后一个合法行为索引，因此，$l_i-k_1$ 其实相当于当前计算奖励值得一个滑动窗口大小。$\omega_k$ 是第 $k$ 个合法行为的权重，合法行为的种类也有四种
...
$$
T_{Blocked} = 2^{-Cr_i} \space\space \text{if } Cr_i < 0
$$
#### 3.3.2 一些考虑
