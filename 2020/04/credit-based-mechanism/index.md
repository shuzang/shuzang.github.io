...
可以将减少阻塞时间作为奖励，但这里遇到的问题是，设备被阻塞后，所有请求都不会被通过，而只有请求被通过，信誉值才会增加，从而减少阻塞时间，这是一个死循环，被阻塞后无法增加信誉值也就无法减少阻塞时间。虽然可以启用其它增加信誉值的方式，但需要权衡，不知道有没有必要。
...
1. RC更名MC（管理合约，Management Contract），负责管理合约和设备属性。
2. MC中设备属性新增 TimeofUnblock 字段，用于设置阻塞终止时间，该字段只能被信誉合约（原JC）更新
3. MC中各种修改参数的函数，非法修改行为产生日志并提交给信誉合约
4. JC更名RC（信誉合约，Reputation Contract），负责计算信誉函数的值，并根据该值计算阻塞终止时间，通过调用MC的相关函数更新设备 TimeofUnblock 字段
5. ACC的访问控制函数首先判断当前时间是否超过阻塞终止时间，确定通过还是拒绝，所有请求日志被提交到PC用于计算
...
- device Register：1
- attribute add：1
- device manager update：2
- device customed attribute update：2
- device delete：3
- attribute delete：3
