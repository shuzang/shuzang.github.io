...
let instance=new TypeIt(`#${group[i]}`,{strings:content,afterComplete:()=>{instance.destroy();typeone(i+1);},}).go();})(0);}}};_Blog.topScrollbar=function(){$(window).scroll(function(){$(".top-scroll-bar").attr("style","width: "+($(this).scrollTop()/($(document).height()-$(this).height())*100)+"%; display: block;");});}
$(document).ready(()=>{_Blog.toggleMobileMenu();_Blog.toggleTheme();_Blog.changeTitle();_Blog.dynamicToTop();_Blog.chroma();_Blog.responsiveTable();_Blog.mermaid();_Blog.echarts();_Blog.countdown();_Blog.typeit();_Blog.toc();_Blog.topScrollbar();});});