# Golang语法基础6-数组、切片与映射


基本的数据类型无法满足所有编程需要，通常情况下，我们还需要一些存储和处理大量数据的高级数据结构，称之为`容器(container)`。一般情况下，容器包括数组、映射、链表三种，但在Go语言中，由于数组不够灵活，增加了切片(slice)类型。Go中切片是频繁使用的，相比来说，数组的使用频率要低得多。

基本的数据类型无法满足所有编程需要，通常情况下，我们还需要一些存储和处理大量数据的高级数据结构，称之为容器(container)。一般情况下，容器包括数组、映射、链表三种，但在Go语言中，由于数组不够灵活，增加了切片(slice)类型。Go中切片是频繁使用的，相比来说，数组的使用频率要低得多。
...
![slice in memory](https://user-images.githubusercontent.com/26682846/68194286-baeef580-ffef-11e9-822c-af854849d163.png)
...
未初始化的map值为nil，如果此时识图给map添加元素会导致运行时错误，因此添加元素必须首先初始化。
