...
随着智能设备数量的急剧增加，物联网近年来得到越来越多的关注和快速发展。它通过现有的网络基础设施有效地将物理世界与Internet集成在一起，以便在智能设备之间共享数据。然而，其复杂的大规模网络结构给物联网系统带来了新的安全风险和挑战。为了保证数据的安全性，传统的访问控制技术由于其复杂的访问管理和集中性导致的可靠性不足，不适合直接用于物联网系统的访问控制。本文提出了一种新的基于属性的用于物联网系统的访问控制方案，大大简化了访问管理。我们使用区块链技术来记录属性的分发，以避免单点故障和数据篡改。访问控制过程也进行了优化，以满足对物联网设备进行高效和轻量化计算的需要。安全性和性能分析表明，该方案能够有效抵御多种攻击，并能在物联网系统中得到有效的实现。
...
一般来说，区块链有三类：公共区块链、联盟区块链和私有区块链。与公共或私有区块链不同，交易验证由联盟区块链中的一组预先选择的节点执行。换句话说，这些节点共同维护公共平台，既不是所有节点在一起，也不是某个节点完全控制。它部分分散，提供了公共区块链的低信任与私有区块链的单一高度信任实体模型之间的混合。与公共区块链相比，联盟区块链用其他有效的共识算法（如实用拜占庭容错（PBFT）和Raft）取代了昂贵的工作量证明（POW)。联盟区块链不需要像比特币一样等待至少六个区块的验证，比特币使用POW作为其共识算法。因此，与公共区块链相比，联盟区块链需要更少的时间达成共识。它使联盟区块链更适合物联网，更具时间敏感性，需要高效的通信。原则上，一旦数据记录到区块链，除非超过51%的节点在公共区块链中通过POW共识算法达成新的共识，否则没有人可以篡改它。但对于联盟区块链，如果出现错误，联盟节点可以在短时间内通过人工干预及时修复。在我们的方案中，负责属性分布的属性权威扮演着这些节点的角色。如果其中一个生成了一个属性交易，那么该交易将首先放入其交易池中。一旦选出领导者，它将把交易打包成一个块，并将该块发布给其他人，以达成共识。一旦到达，新块将被附加到区块链的末尾。
...
![两个通信实体的访问控制](https://user-images.githubusercontent.com/26682846/59078893-c12f5780-8913-11e9-92f9-6692ec0d89bc.png)
